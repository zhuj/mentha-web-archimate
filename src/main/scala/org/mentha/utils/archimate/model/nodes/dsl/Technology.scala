...
object Technology {
  ...
  implicit class ImplicitCommunicationNetwork(src: CommunicationNetwork)(implicit val model: Model) {
    ...
    def `aggregates`(dst: Grouping): AggregationRelationship = _aggregates(src, dst)(model)
    def `aggregates`(dst: SystemSoftware): AggregationRelationship = _aggregates(src, dst)(model)
    @derived def `aggregates`(dst: Equipment): AggregationRelationship = _aggregates(src, dst)(model)
    @derived def `aggregates`(dst: Facility): AggregationRelationship = _aggregates(src, dst)(model)
